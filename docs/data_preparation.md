...
Then continue with [Training](training.md#choose-an-entry-point-and-recipe).
...
The archives are hosted in the [`VOXXXX1874/Hista`](https://huggingface.co/datasets/VOXXXX1874/Hista) dataset repository. Install the Hugging Face Hub CLI and download both files from the Hista repository root:
...
You can also download only the archive required by your workflow. Both archives contain repository-relative directory trees; use the extraction locations below rather than unpacking them into an arbitrary directory.

#### Extra Rollouts

`extra_rollouts.tar.gz` contains the additional responses generated by the reference Qwen2.5 models before difficulty selection. It includes merged rollout files such as:

```text
data_preparation/
├── DAPO-17K/
│   └── 3B/sampled/sample_20.json
├── openr1-220K/
│   └── 3B/sampled/sample_20.json
├── MixtureOfThought/
│   └── 3B/sampled/sample_20.json
...
The archive also contains the available rollout artifacts for other released model scales. Each `sample_20.json` stores 20 verified responses per selected problem, separated into `correct_responses` and `wrong_responses`. Downloading it avoids the expensive model rollout and verifier stage described in [Sample and Align Other Training Data](#sample-and-align-other-training-data).

The archive root is `data_preparation`, so extract it into `src`:

```bash
tar -xzf tmp/hista_data/extra_rollouts.tar.gz -C src
```

This produces paths such as `src/data_preparation/DAPO-17K/3B/sampled/sample_20.json`. Existing files at the same paths are overwritten.

The downloaded files are already merged across the original `partition_*` outputs. Therefore, skip both extra sampling and the `merge_samples.py` commands at the beginning of each block in `scripts/process/select_*.sh`. Run the subsequent `select_samples.py` commands to construct the difficulty tiers under `sampled/<tier>/train.json` and, where needed, `sampled_numca/<tier>/train.json`.

#### SVEB data & Hybrid dataset

`sveb_hybrid_data.tar.gz` is a snapshot created directly from the repository-level `data` directory. It contains ready-to-use artifacts including:

```text
data/
├── MATH-500/                    # processed evaluation benchmarks and configs
├── GSM8K/
├── AIME2425/
├── ...
├── 1.5B/
│   ├── hybrid/                  # train.json and test.json for hybrid RL
│   ├── sveb_math/
│   ├── sveb_number/
│   ├── sveb_science/
│   ├── sveb_general/
...
Because the archive already begins with `data/`, this populates the expected training, SVEB, and benchmark paths without another move or copy. Existing files under `data` with matching names are overwritten, while unrelated files remain in place.
