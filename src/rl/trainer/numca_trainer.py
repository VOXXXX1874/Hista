...
class NumcaTrainer(GRPOPlusTrainer):
    ...
    def _generate_and_score_completions(
        ...
        if mode == "train":
            ...
            advantages, rewards_per_func = self._calculate_Numca_advantages(
                inputs, 
                prompts, 
                completions, 
                completion_ids, 
    ...
    @profiling_decorator
    def _calculate_Numca_advantages(
        self, 
        ...
        distinct_ids = set()
        rewards = (rewards_per_func * self.reward_weights.to(device).unsqueeze(0)).nansum(dim=1)
        completions = [completion[0]["content"] if is_conversational({"prompt": completion}) else completion for completion in completions]
        for i, (completion, one_input) in enumerate(zip(completions, inputs)):
            # Extract the number from the expressions in completions text
            expressions, positions = number_parse(completion)
            ...
            # Map character end-positions (positions) to token indices.
            # For each character position, find the first token whose end >= pos.
            # If none found, assign the last token index. Then clamp to the generation length later.
            token_positions = []
            for char_pos in positions:
                found = False
                for token_idx, (_, end) in enumerate(offsets):
                    if char_pos < end:
                        token_positions.append(token_idx + 1)  # use token-count style positions
                        found = True
                        break
                if not found:
                    token_positions.append(len(offsets))
            # Replace character positions with token positions
            positions = token_positions
            expressions_rewards_pair.append((one_input["id"], expressions, positions, rewards[i].item()))
            distinct_ids.add(one_input["id"])
        # Gather the expressions_rewards_pair and update the NumCA dictionary
        all_expressions_rewards_pair = gather_object(expressions_rewards_pair)
        for id, expressions, positions, reward in all_expressions_rewards_pair:
            if id not in distinct_ids:
                continue
            if id not in self.numca_dict:
                self.numca_dict[id] = Numca_dict(expressions, reward)
            else:
                self.numca_dict[id].update(expressions, reward)
        # According to the updated NumCA dictionary, allocate the advantages for each generation
        advantages_list = []
        for i, expressions_reward in enumerate(expressions_rewards_pair):
            id, expressions, positions, reward = expressions_reward
            advantages = self.numca_dict[id].advantages(
                expressions, positions, completion_ids.size(1), self.gae_lambda, final_reward=reward
            )
        ...
        if self.log_completions and self.state.global_step % self.args.logging_steps == 0:
            ...
            if self.accelerator.is_main_process:
                for i, advantages in enumerate(advantages_list):
                    ...
                    print('-' * 100)
                    if expressions_rewards_pair[i][0] != -1:  
                        print('Initial state value function:', self.numca_dict[expressions_rewards_pair[i][0]].root_node.state_value)
        ...
        advantages_list = torch.stack(advantages_list)
